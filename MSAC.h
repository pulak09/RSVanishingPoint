...
private:
	...
	// Consensus set
	std::vector<int> __CS_idx, __CS_idx1, __CS_idx2, __CS_idx3, __CS_best;	// Indexes of line segments: 1 -> belong to CS, 0 -> does not belong 
...
public:
	...
	void multipleVPEstimation(std::vector<std::vector<cv::Point> > &lineSegments, std::vector<std::vector<std::vector<cv::Point> > > &lineSegmentsClusters, std::vector<int> &numInliers, cv::Mat &vps, int numVps);
		
	/** Draws vanishing points and line segments according to the vanishing point they belong to*/
	void drawCS(cv::Mat &im, std::vector<std::vector<std::vector<cv::Point> > > &lineSegmentsClusters, cv::Mat &vps);
	void RotwarpImage(cv::Mat &im, cv::Mat &vps); 
private:	
	/** This function returns a randomly selected MSS*/
	void GetMinimalSampleSet(cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, std::vector<int> &MSS, std::vector<cv::Mat> &vp, int iter);
	...
	void estimateLS(cv::Mat &Li, cv::Mat &Lengths, std::vector<int> &set, int set_length, std::vector<cv::Mat> &vEst);	

	void estimateRS(cv::Mat &Li, cv::Mat &Lengths, std::vector<int> &set, int set_length, std::vector<cv::Mat> &vEst);
